...
class VentaController extends Controller
{
    ...
    private function persistAnulacionAuditForVenta(?Venta $venta, array $auditData, ?string $message = null): void
    {
        ...
        $cartRow = DB::table('facturacion_carts')
        ...
        $isPaidQrCart = $cartRow
            && strtolower(trim((string) ($cartRow->metodo_pago ?? ''))) === 'qr'
            && strtolower(trim((string) ($cartRow->estado_pago ?? 'pendiente'))) === 'pagado'
            && trim((string) ($cartRow->qr_transaction_id ?? '')) !== '';
        ...
        if ($estadoSufe === 'ANULADA') {
            if ($isPaidQrCart) {
                $cartUpdates['estado'] = 'emitido';
                $cartUpdates['estado_emision'] = 'NO_APLICA';
                $cartUpdates['codigo_seguimiento'] = null;
                $cartUpdates['codigo_seguimiento_fiscal'] = null;
            } else {
    ...
    private function facturacionCartStatusPayload(object $cart, ?object $linkedVenta = null): array
    {
        ...
        if ($isQrTrackedSale) {
            ...
            if ($isLinkedVentaAnnulmentObserved || $estadoEmision === 'ANULACION_OBSERVADA') {
                return $this->makeStatusPayload('cart', 'ANULACION_OBSERVADA', [
                    'can_consult' => true,
                    'cuf' => $cuf !== '' ? $cuf : null,
                ]);
            }
    ...
    private function canConsultFacturacionCart(object $cart): bool
    {
        ...
        if ($codigoSeguimiento !== '' && in_array($estadoEmision, ['PENDIENTE', 'ERROR', 'RECHAZADA', 'ANULACION_SOLICITADA', 'ANULACION_OBSERVADA'], true)) {
            return true;
        }
    ...
    ): array
    {
        ...
        return [
            ...
            'qrCancelacion' => [
                ...
                'canceladaPorNombre' => $cart->qr_cancelado_por_nombre ?? null,
                'canceladaPorEmail' => $cart->qr_cancelado_por_email ?? null,
                'motivo' => $cart->qr_cancelacion_motivo ?? null,
                'origen' => $cart->qr_cancelacion_origen ?? null,
                'transactionId' => $cart->qr_cancelacion_transaction_id ?? $cart->qr_transaction_id ?? null,
                'mensaje' => $cart->qr_cancelacion_mensaje ?? $cart->mensaje_emision ?? null,
                'codigoOrden' => $this->normalizeFacturacionCartCodigoOrden($cart),

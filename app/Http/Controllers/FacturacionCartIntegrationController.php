...
class FacturacionCartIntegrationController extends Controller
{
    ...
    public function emitir(Request $request): JsonResponse
    {
        ...
        $codigoOrdenEmitido = $isPaidQrInvoiceConversion
    ...
    private function payloadFromCart(object $cart, $items): array
    {
        ...
        $isPaidQrInvoiceConversion = strtolower((string) ($cart->metodo_pago ?? 'efectivo')) === 'qr'
            && strtolower((string) ($cart->estado_pago ?? 'pendiente')) === 'pagado'
            && trim((string) ($cart->qr_transaction_id ?? '')) !== '';
        $codigoOrden = $isPaidQrInvoiceConversion
            ? trim((string) ($cart->codigo_orden ?? ''))
            : $this->normalizeBridgeCodigoOrden($cart->codigo_orden ?? null, 'factura_electronica');
        if ($codigoOrden === '') {
    ...
    private function cartById(int $id): ?array
    {
        ...
        $historialQr = [];
        if (is_array($qrReemision) && (
            !empty($qrReemision['factura_anulada_numero'])
            || !empty($qrReemision['reemitida_desde_codigo_orden'])
            || !empty($qrReemision['factura_anulada_cuf'])
        )) {
            $historialQr[] = [
                'id' => $qrReemision['venta_anulada_id'] ?? null,
                'codigoOrden' => $qrReemision['reemitida_desde_codigo_orden'] ?? null,
                'codigoSeguimiento' => null,
                'numeroFactura' => $qrReemision['factura_anulada_numero'] ?? null,
                'cuf' => $qrReemision['factura_anulada_cuf'] ?? null,
                'fecha' => $qrReemision['reemitida_at'] ?? null,
                'total' => (float) ($c->total ?? 0),
                'qr_transaction_id' => $c->qr_transaction_id ?? null,
                'estadoSufe' => $qrReemision['factura_anulada_estado'] ?? 'ANULADA',
                ...
                'anulacion' => [
                    ...
                    'cuf' => $qrReemision['factura_anulada_cuf'] ?? null,
                    'estadoSufe' => $qrReemision['factura_anulada_estado'] ?? 'ANULADA',
                    'motivo' => $qrReemision['motivo'] ?? null,
                ],
            ];
        }
        ...
        if ($isQrTrackedSale) {
            ...
            if ($hasActiveFiscalAttempt) {
                return match ($estadoEmision) {
                    'FACTURADA' => $this->makeFacturacionCartStatusPayload('FACTURADA', [
                        'can_annul' => $canAnnul,
                        'cuf' => $cuf !== '' ? $cuf : null,
                    ]),
                    'PENDIENTE' => $this->makeFacturacionCartStatusPayload('PENDIENTE', [
                        'can_consult' => true,
                        'cuf' => $cuf !== '' ? $cuf : null,
                    ]),
                    'RECHAZADA' => $this->makeFacturacionCartStatusPayload('RECHAZADA', [
                        'can_consult' => true,
                        'cuf' => $cuf !== '' ? $cuf : null,
                    ]),
                    'ERROR' => $this->makeFacturacionCartStatusPayload('ERROR', [
                        'can_consult' => true,
                        'cuf' => $cuf !== '' ? $cuf : null,
                    ]),
                    default => $this->makeFacturacionCartStatusPayload($estadoEmision, [
                        'can_consult' => true,
                        'cuf' => $cuf !== '' ? $cuf : null,
                    ]),
                };

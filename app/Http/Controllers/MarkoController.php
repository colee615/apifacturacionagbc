...
class MarkoController extends Controller
{
   public function prueba(Request $request)
   {
      try {
         ...
         $validated = $request->validate([
            ...
            'TELEFONODESTINATARIO' => 'nullable|string',
         ]);

         // Obtener los datos validados del request
         $importe = 0;
         $provincia = strtoupper($validated['PROVINCIA']);
         $peso = $validated['PESO'];
         ...
         if ($peso >= 0.001 && $peso <= 0.020) {
            ...
            $importe = $precio * $cantidad;
         } elseif ($peso > 19.001 && $peso <= 20.000) {
            $precio = $this->calcularPrecio($provincia, $peso, [
               'LOCAL 1' => 57,
               'LOCAL 2' => 58,
               'LOCAL 3' => 59,
         ...
         if (strlen($validated['TELEFONODESTINATARIO']) > 1) {
            $importe += 2;
         }

         return response()->json(['IMPORTE' => $importe], 200);
      } catch (\Exception $e) {
         // Registrar el error
         Log::error('Error en MarkoController@store: ' . $e->getMessage());
